...
del fulldata['points'] ### either margin, points, allowed or share!!!!!!!!!!!!!!!
...
#plt.show()
##
#clist.index(max(clist[20:30]))
#
#for i in [2, 6, 8, 17, 26, -1]:
#    Z = linkage(X[indices[:i]].T, 'ward')
#    c, coph_dists = cophenet(Z, pdist(X[indices[:i]].T))
#    print(c)    
#    last = Z[-10:, 2]
...
r2list = []
sillist = []
scoreresults = resample(scoreresults, n_samples = int(len(fulldata)/3), random_state = 46)
loopX = resample(fulldata, n_samples = int(len(fulldata)/3), random_state = 46)
...
for i in clustsizes:
    for k in numfeats:
        ...
        cvscores = cross_validate(SVC(kernel='rbf'), pcadf, scoreresults, scoring =('neg_mean_absolute_error', 'r2'), cv = KFold(n_splits = 10, random_state = 86))
